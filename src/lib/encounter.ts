...
export function generateEncounter({
  ...
  const results: Array<{ name: string; cr: string; xp: number }> = [];
  let remaining = budget;
  let safety = 0;

  while (remaining > 0 && safety < 30) {
    safety += 1;
    const affordable = entries.filter((entry) => entry.xp <= remaining);
    if (affordable.length === 0) break;
    const pick = affordable[Math.floor(Math.random() * affordable.length)];
    results.push({ name: pick.name, cr: pick.cr, xp: pick.xp });
    remaining -= pick.xp;
  }
