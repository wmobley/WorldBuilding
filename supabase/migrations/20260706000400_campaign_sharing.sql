...
drop policy if exists assets_select_shared on assets;
create policy assets_select_shared on assets
  for select
  using (
    is_campaign_owner(assets.campaign_id)
    or (
      is_campaign_member(assets.campaign_id)
      and exists (
        select 1
        from docs d
        left join folders f on f.id = d.folder_id
        where d.id = assets.doc_id
          and (d.shared = true or f.shared = true)
      )
    )
  );

drop policy if exists assets_write_owner on assets;
create policy assets_write_owner on assets
  for all
  using (
    is_campaign_owner(assets.campaign_id)
  )
